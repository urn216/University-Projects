...
public class Parser {
  ...
  public static Collection<Restriction> parseRestrict(
  File segments,
  Graph graph)
  {
    try{
      final Function<String[], Restriction> toSegment =
      tokens -> {
        int road1ID = asInt(tokens[1]),
        road2ID = asInt(tokens[3]);
        int node1ID = asInt(tokens[0]),
        node2ID = asInt(tokens[2]),
        node3ID = asInt(tokens[4]);

        return new Restriction(graph, road1ID, road2ID, node1ID, node2ID, node3ID);
      };
