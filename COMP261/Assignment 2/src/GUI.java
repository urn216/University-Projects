...
public abstract class GUI {
	...
	/**
	 * Is called whenever the FindCrits button is pressed.
	 * Finds all articulatuion points within a supplied graph.
	 */
	protected abstract void onArtPts();

	/**
	 * Is called whenever the from button is pressed.
	 * Sets whatever the targeted node is to the departure node.
	 */
	protected abstract void onFrom();

	/**
	 * Is called whenever the destination button is pressed.
	 * Sets whatever the targeted node is to the destination node.
	 */
	protected abstract void onTo();

	/**
	 * Is called whenever the use dist button is pressed.
	 * Sets the A* mode to use distance.
	 */
	protected abstract void onDist();

	/**
	 * Is called whenever the use time button is pressed.
	 * Sets the A* mode to use time.
	 */
	protected abstract void onTime();

	/**
	 * Is called whenever the plot trip button is pressed.
	 * Uses A* search to find the shortest route between the from and to positions.
	 */
	protected abstract void onJourney();
	...
	/**
	 * Is called when the mouse is scrolled, and is passed the MouseWheelEvent object for that scroll.
	 */
	...
	private void initialise() {
		...
		JButton setFrom = new JButton("Mark From");
		setFrom.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				onFrom();
				redraw();
			}
		...
		JButton setTo = new JButton("Mark Dest");
		setTo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				onTo();
				redraw();
			}
		...
		JButton setDist = new JButton("Use Dist");
		setDist.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				onDist();
			}
		});

		JButton setTime = new JButton("Use Time");
		...
		JButton aStar = new JButton("Plan Route");
		...
		JButton findCrits = new JButton("Critical Points");
		findCrits.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				onArtPts();
				redraw();
			}
		...
		extras.setMaximumSize(new Dimension(150, 60));
		extras.setLayout(new GridLayout(2, 3));
		extras.add(setFrom);
		extras.add(setDist);
		extras.add(aStar);
		extras.add(setTo);
		extras.add(setTime);
		extras.add(findCrits);
		controls.add(extras);
		controls.add(Box.createRigidArea(new Dimension(15, 0)));
