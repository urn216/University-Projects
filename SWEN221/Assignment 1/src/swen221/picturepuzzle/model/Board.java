...
public class Board {
	...
	private Cell[] cells; // 
	/** 
	 * The model cells to base the answer off of 
	 */
	private final Cell[] ans;
	...
	public Board(int[] image, int width, int dimension) {
		...
		this.ans = cells.clone();
	}

	/**
	 * Get size of this board.
	 *
	 * @return Board size integer.
	 */
	 ...
	 * This includes the cells being in the right place and all being oriented correctly.
	 *
	 * @return True if the board is completed.
	 */
	public boolean isFinished() {
		if (cells[cells.length-1] != null) {return false;}
		for (int i = 0; i < cells.length-1; i++) {
			if (cells[i].isRotated() || cells[i]!=ans[i]) {return false;}
		}
		return true;
	}
	...
	public void swap(Location l1, Location l2) {
		int id1 = (l1.row * dimension) + l1.col;
		int id2 = (l2.row * dimension) + l2.col;
	 ...
	 *
	 * @return A string composed of '-' characters.
	 */
	 ...
	 * 
	 * @param image The array containing the image for the board.
	 * 
	 * @param width The width of the image.
	 * 
	 * @param dimension The number of cells in either dimension
	 *
	 ...
	 * @return Cell     --- The cell that was carved out.
	 */
